...
namespace NavData_Interface.DataSources.DFDUtility.Factory
{
    internal class StarFactory
    {
        public static Star Factory(SQLiteDataReader reader, SQLiteConnection connection)
        {
            ...
            return new Star(airportIdentifier, routeIdentifier, transitions, commonLegs, runwayTransitions, transitionAltitude);
        }

        private static Transition ReadTransition(SQLiteDataReader reader, SQLiteConnection connection, string transitionIdentifier)
        {
            // The transition altitude for this STAR is always on the first leg of each transition. Store it now
